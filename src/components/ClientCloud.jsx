...
function Review({ title, body, author, rating, className, ...props }) {
  ...
  return (
    <figure
      className={clsx(
        'bg-t animate-fade-in rounded-3xl p-6 opacity-0 shadow-md shadow-gray-900/5',
        className
      )}
...
function ReviewGrid() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 top-0 h-32 pointer-events-none bg-gradient-to-b from-pink-50" />
      <div className="absolute inset-x-0 bottom-0 h-32 pointer-events-none bg-gradient-to-t from-pink-50" />
    </div>
  )
}
...
export function ClientCloud() {
  return (
    <section
      ...
      className="pt-20 pb-16 bg-t sm:pt-32 sm:pb-24"
    >
      <Container>
        <h2
          id="reviews-title"
          className="mt-2 text-3xl font-bold tracking-tight text-center text-gray-900 sm:text-4xl"
        >
          LoopFilm AS: The go-to production company
          <br />
          for filmmakers in Norway
        </h2>
        <p className="max-w-2xl mx-auto mt-6 text-lg leading-8 text-center text-gray-600">
          Our past clients speak for themselves:
        </p>
        <ReviewGrid />
      </Container>
    </section>
